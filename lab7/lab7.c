...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  for (int i = 0; i < *result_count; i++) {
    if (input->doubled_value == output[i].doubled_value) {
      // We need to append the input line number to the end of the corresponding
      // linenumbers array
      int idx = output[i].count;
      output[i].line_numbers[idx] = input->line_number;
      output[i].count++;
      return;
    }
  }
  // line_numbers list If it doesn't exist, add a new entry to the output
  // this is the end of output array because we couldnt find the
